...
get_ds获得kernel的内存访问地址范围（IA32是4GB），set_fs是设置当前的地址访问限制值，
get_fs是取得当前的地址访问限制值。进程由用户态进入核态，
linux进程的task_struct结构中的成员addr_limit也应该由0xBFFFFFFF变为0xFFFFFFFF
(addr_limit规定了进程有用户态核内核态情况下的虚拟地址空间访问范围，在用户态，
addr_limit成员值是0xBFFFFFFF也就是有3GB的虚拟内存空间，在核心态，是0xFFFFFFFF,范围扩展了1GB)。
...
/****************************************内核函数传参************************************
1 内核函数调用系统调用，参数是由内核传入系统调用函数，而不是由用户空间，内核函数传入文件名指针char *，
  需要在内核函数中将文件名传入系统调用，需将文件名从用户空间拷贝进内核空间
